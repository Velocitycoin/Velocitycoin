...
unsigned int nStakeTargetSpacing = 30; //  32 second block spacing
int64 nChainStartTime = 1387769316;
...
const string strMessageMagic = "Velocitycoin Signed Message:\n";
...
// Velocitycoin: increasing Nfactor gradually
...
int64 GetProofOfWorkReward( int nHeight, uint256 prevHash)
{
        ...
        int rand = generateMTRandom(seed, 10000);
        int rand1 = 0;
        int rand2 = 0;
        int rand3 = 0;
        int rand5 = 0;
        if(nHeight < 5000)  
            {   
                if(rand < 1000)
                nSubsidy = rand + 1000 * COIN;
                else if(rand > 1000)
                nSubsidy = rand * COIN;  
            }  
        else if(nHeight < 10000)  
            {   
            cseed_str = prevHash.ToString().substr(5,7);
                cseed = cseed_str.c_str();
                seed = hex2long(cseed);
                rand1 = generateMTRandom(seed, 5000);
                if(rand1 < 1000)
            ...
            cseed_str = prevHash.ToString().substr(5,7);
                cseed = cseed_str.c_str();
                seed = hex2long(cseed);
                rand2 = generateMTRandom(seed, 1500);
                if(rand2 < 500)
                nSubsidy = rand2 + 100 * COIN;
                else if(rand2 > 500)
                nSubsidy = rand2 * COIN;  
            }
        ...
        else if(nHeight < 20000)      
        {
                ...
                cseed = cseed_str.c_str();
                seed = hex2long(cseed);
                rand3 = generateMTRandom(seed, 500);
                if(rand3 < 100)
                nSubsidy = rand3 + 100 * COIN;
                else if(rand3 > 100)
                nSubsidy = rand3 * COIN;
        }
        ...
        else if(nHeight < 25000)      
        {
                ...
                cseed = cseed_str.c_str();
                seed = hex2long(cseed);
                rand5 = generateMTRandom(seed, 200);
                if(rand5 < 50)
                nSubsidy = rand5 + 100 * COIN;
                else if(rand5 > 50)
                nSubsidy = rand5 * COIN;
        }
...
static const int64 nTargetTimespan = 60 * 60;  // one day
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Velocitycoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
  ...
  //vMerkleTree: 05b4eb8ba8 
        ...
        // Genesis block
        const char* pszTimestamp = "Velocitycoin is quicker than greaded lightning";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0xa83963c4b71a79a62e69aaf7e429a7689c69a8c95ff26b40dac4fad337659025"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe9, 0xd6, 0xf7, 0xe5 };
